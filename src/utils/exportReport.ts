...
export function generateCompanyExpenseReportCSV(
  ...
  csvContent += `Tanggal,:,${header.reportDate}\n`;
  csvContent += `Project,:,${header.projectName}\n\n`;

  // 2. Header Tabel
  csvContent += `TANGGAL,NO,KETERANGAN (NAMA BARANG),JUMLAH ITEM,KATEGORI,HARGA SATUAN (Rp),TOTAL (Rp)\n`;
  ...
  sortedTx.forEach((tx) => {
    ...
    if (tx.items && tx.items.length > 0) {
      // 1 Struk memuat banyak item baris belanja
      tx.items.forEach((item) => {
        ...
        csvContent += `"${txDateStr}",${rowNumber},${cleanItemName},${qtyStr},"${catName}",${item.unit_price || 0},${itemTotal}\n`;
        rowNumber++;
      });
    } else {
      ...
      csvContent += `"${txDateStr}",${rowNumber},"${tx.merchant_name}","1 Paket","${catName}",${total},${total}\n`;
      rowNumber++;
    }
  });
